...
export async function POST(request: Request) {
  ...
  try {
    ...
    const body = await request.json() as { projectId?: string; plan?: "team" | "enterprise" };
    ...
    const plan = body.plan === "enterprise" ? "enterprise" : "team";
    return withWorkspaceIdentity(Response.json(await createCheckoutSession({ organizationId: organization.id, plan, email: identity.account?.email ?? null, origin: new URL(request.url).origin })), identity);
  } catch (error) {
    const status = error instanceof BillingError || error instanceof OrganizationError ? error.status : 500;
    return withVisitorSession(Response.json({ error: error instanceof Error ? error.message : "创建 Stripe Checkout 失败" }, { status }), visitor);
  }
}
