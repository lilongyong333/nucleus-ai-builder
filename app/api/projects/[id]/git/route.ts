...
export async function PUT(request: Request, context: { params: Promise<{ id: string }> }) {
  return handle(request, context, async (id) => {
    ...
    return { integration: await configureGitIntegration(id, { repositoryOwner: body.repositoryOwner, repositoryName: body.repositoryName, defaultBranch: body.defaultBranch }) };
  });
}

async function handle(request: Request, context: { params: Promise<{ id: string }> }, action: (id: string) => Promise<unknown>) {
  ...
  try {
    ...
    return withWorkspaceIdentity(Response.json(await action(id)), identity);
  } catch (error) {
