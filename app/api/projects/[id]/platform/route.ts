...
export async function GET(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const [manifest, stats, evidence, backups, runnerJobs, organization, approvals, git] = await Promise.all([
      getAppManifest(id),
    ...
    return withWorkspaceIdentity(Response.json({ manifest, stats, evidence, backups, runnerJobs, organization, approvals, git }), identity);
  } catch (error) {
