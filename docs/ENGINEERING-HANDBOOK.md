...
- 单次请求超时；
- 空回复。
- SSE 没有正常结束事件，或提供商返回 `finish_reason=length` 的截断回复。

401 和 429 不自动切换，因为密钥错误和整体限流通常不是换模型能解决的。用户取消会直接传播 AbortSignal，不再调用备用模型。

模型返回内容后还不能直接建立检查点。平台会先执行三文件协议门：Bob 的文件职责由代码固定；`index.html` 禁止内联样式与脚本；CSS 必须括号完整；JavaScript 必须通过 Acorn 语法解析。协议失败只记录本次模型尝试并重试当前阶段，不污染已完成工件。
