...
| 能力 | 当前 | 企业目标 |
|---|---|---|
| 分支/PR | 基线曾直接 push；新增文档走 PR | 所有日常变化必须 PR |
| CI | 本地完整检查 | GitHub Actions required checks |
| 环境 | local + production | preview/staging/production |
| 用户 | 匿名 | 登录、所有权、RBAC |
| 数据 migration | SQL + 运行期防御初始化 | 部署前 migration gate |
| 监控 | 手工线上验收 | 指标、日志、追踪、告警 |
| 模型 | 单默认模型、轻量重试 | 路由、超时、熔断、预算 |
| 安全 | sandbox、secret、限流 | CSP、WAF、审核、审计 |
...
6. 公开源码链接；
7. 公网生成限流；
8. 教学文档使用独立分支和 Draft PR。

这比“最后一次性上传成品”更能说明工程过程。每个修复都来自可复现的本地或线上失败。
