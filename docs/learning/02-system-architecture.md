...
flowchart LR
    ...
    W -->|"OpenAI-compatible API"| M["OpenCode Go 模型"]
...
sequenceDiagram
    ...
    A->>D: 检查限流、读取项目、标记 generating
    A-->>B: status(Iris working)
    A->>M: 生成结构化计划
    M-->>A: AgentPlan JSON
    A-->>B: plan + status events
    A->>M: 生成三个 path code blocks
    M-->>A: HTML/CSS/JS
    A->>A: 解析、合并、校验、必要时补文件
    A->>D: 保存 Version 全量快照
    A-->>B: file events + complete(Project)
    B->>B: 更新 React 状态
    B->>I: 注入组合后的 srcDoc
    I-->>B: 错误时 postMessage
```
...
| AI | `lib/opencode.ts` | 模型配置、Prompt、重试、补全 |
| 解析/运行时 | `lib/parser.ts`、`lib/runtime.ts` | 解析文件、校验、组装预览 |
| 数据 | `lib/db.ts`、`db/schema.ts` | D1 查询、快照、发布、限流 |
