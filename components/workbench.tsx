    ...
    <main className={`workbench ${sidebarOpen ? "" : "sidebar-collapsed"}`}>
      ...
      <section className="canvas-panel">
        ...
        {activeTab === "preview" ? <div className={`preview-stage ${device}`}>
          ...
          {selectedElement && <section className="visual-edit-panel"><header><div><MousePointer2 size={14} /><span>局部可视化修改</span></div><button onClick={() => setSelectedElement(null)} aria-label="关闭局部修改"><X size={13} /></button></header><code>{selectedElement.selector}</code><div className="visual-fields"><label className="visual-wide">文字<input value={visualText} onChange={(event) => setVisualText(event.target.value)} /></label><label>背景<input value={visualBackground} onChange={(event) => setVisualBackground(event.target.value)} placeholder="#ffffff" /></label><label>文字色<input value={visualColor} onChange={(event) => setVisualColor(event.target.value)} placeholder="#111111" /></label><label>圆角<input value={visualRadius} onChange={(event) => setVisualRadius(event.target.value)} placeholder="12px" /></label><label>字号<input value={visualFontSize} onChange={(event) => setVisualFontSize(event.target.value)} placeholder="16px" /></label><label>内边距<input value={visualPadding} onChange={(event) => setVisualPadding(event.target.value)} placeholder="12px 16px" /></label><label>Gap<input value={visualGap} onChange={(event) => setVisualGap(event.target.value)} placeholder="8px" /></label><label>宽度<input value={visualWidth} onChange={(event) => setVisualWidth(event.target.value)} placeholder="100% / 320px" /></label><label>对齐<input value={visualAlign} onChange={(event) => setVisualAlign(event.target.value)} placeholder="left / center" /></label></div><footer><button onClick={resetVisualPreview}><RotateCcw size={11} />撤销</button><button onClick={applyVisualPreview}>预览修改</button><button className="primary" onClick={persistVisualEdit} disabled={busy}><Sparkles size={12} />写入新版本</button></footer></section>}
