...
type PlatformState = {
  ...
  databaseResource: AppDatabaseResource | null;
...
export function Workbench({ projectId }: { projectId: string }) {
  ...
  const [visualFontSize, setVisualFontSize] = useState("");
  const [visualPadding, setVisualPadding] = useState("");
  const [visualGap, setVisualGap] = useState("");
  const [visualWidth, setVisualWidth] = useState("");
  const [visualHeight, setVisualHeight] = useState("");
  const [visualMargin, setVisualMargin] = useState("");
  const [visualDisplay, setVisualDisplay] = useState("");
  const [visualColumns, setVisualColumns] = useState("");
  const [visualAlign, setVisualAlign] = useState("");
  ...
  useEffect(() => {
    const receive = (event: MessageEvent) => {
      ...
      if (event.data.type === "element-selected" && event.data.element && typeof event.data.element === "object") {
        ...
        setVisualFontSize(element.styles?.fontSize ?? "");
        setVisualPadding(element.styles?.padding ?? "");
        setVisualGap(element.styles?.gap ?? "");
        setVisualWidth(element.styles?.width ?? "");
        setVisualHeight(element.styles?.height ?? "");
        setVisualMargin(element.styles?.margin ?? "");
        setVisualDisplay(element.styles?.display ?? "");
        setVisualColumns(element.styles?.gridTemplateColumns ?? "");
        setVisualAlign(element.styles?.textAlign ?? "");
      }
    };
  ...
  function applyVisualPreview() {
    ...
    iframeRef.current?.contentWindow?.postMessage({
      ...
      styles: {
        ...
        ...(visualFontSize.trim() ? { fontSize: visualFontSize.trim() } : {}),
        ...(visualPadding.trim() ? { padding: visualPadding.trim() } : {}),
        ...(visualGap.trim() ? { gap: visualGap.trim() } : {}),
        ...(visualWidth.trim() ? { width: visualWidth.trim() } : {}),
        ...(visualHeight.trim() ? { height: visualHeight.trim() } : {}),
        ...(visualMargin.trim() ? { margin: visualMargin.trim() } : {}),
        ...(visualDisplay.trim() ? { display: visualDisplay.trim() } : {}),
        ...(visualColumns.trim() ? { gridTemplateColumns: visualColumns.trim() } : {}),
        ...(visualAlign.trim() ? { textAlign: visualAlign.trim() } : {}),
      },
    }, "*");
  ...
  function resetVisualPreview() {
    if (!selectedElement) return;
    setVisualText(selectedElement.text);
    setVisualBackground(selectedElement.styles.backgroundColor ?? "");
    setVisualColor(selectedElement.styles.color ?? "");
    setVisualRadius(selectedElement.styles.borderRadius ?? "");
    setVisualFontSize(selectedElement.styles.fontSize ?? "");
    setVisualPadding(selectedElement.styles.padding ?? "");
    setVisualGap(selectedElement.styles.gap ?? "");
    setVisualWidth(selectedElement.styles.width ?? "");
    setVisualHeight(selectedElement.styles.height ?? "");
    setVisualMargin(selectedElement.styles.margin ?? "");
    setVisualDisplay(selectedElement.styles.display ?? "");
    setVisualColumns(selectedElement.styles.gridTemplateColumns ?? "");
    setVisualAlign(selectedElement.styles.textAlign ?? "");
    iframeRef.current?.contentWindow?.postMessage({ source: "nucleus-host", type: "visual-patch", selector: selectedElement.selector, text: selectedElement.text, styles: selectedElement.styles }, "*");
    setNotice("已撤销本次本地预览修改");
  }
  ...
  function persistVisualEdit() {
    ...
    const prompt = [
      ...
      `目标字号：${visualFontSize || "保持原值"}`,
      `目标内边距：${visualPadding || "保持原值"}`,
      `目标间距：${visualGap || "保持原值"}`,
      `目标宽度：${visualWidth || "保持原值"}`,
      `目标高度：${visualHeight || "保持原值"}`,
      `目标外边距：${visualMargin || "保持原值"}`,
      `目标 display：${visualDisplay || "保持原值"}`,
      `目标网格列：${visualColumns || "保持原值"}`,
      `目标对齐：${visualAlign || "保持原值"}`,
  ...
  async function provisionDatabase(silent = false) {
    const response = await fetch(`/api/projects/${projectId}/database`, { method: "POST" });
    const data = await response.json() as { resource?: AppDatabaseResource; error?: string };
    if (!silent) {
      setNotice(response.ok
        ? data.resource?.status === "ready" ? "独立 D1 已创建并完成 Schema Migration" : "Provisioner 已登记；生产凭据待配置"
        : data.error || "独立数据库创建失败");
    }
    if (showPlatform) await loadPlatform();
  }
  ...
  async function inviteMember() {
    ...
    const data = await response.json() as { error?: string; delivery?: NotificationDelivery };
    setNotice(response.ok ? data.delivery?.status === "sent" ? "协作者邀请已记录，邀请邮件已发送" : "邀请已记录；邮件服务待配置，对方仍可用同一邮箱登录认领" : data.error || "邀请失败");
  ...
  async function openCheckout(plan: "team" | "enterprise") {
    const response = await fetch("/api/billing/checkout", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ projectId, plan }) });
    const data = await response.json() as { url?: string; error?: string };
    if (!response.ok || !data.url) return setNotice(data.error || "Stripe Checkout 创建失败");
    window.location.assign(data.url);
  }

  async function openBillingPortal() {
    const response = await fetch("/api/billing/portal", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ projectId }) });
    const data = await response.json() as { url?: string; error?: string };
    if (!response.ok || !data.url) return setNotice(data.error || "账单门户打开失败");
    window.location.assign(data.url);
  }
  ...
  useEffect(() => {
    ...
    if (!versionId || project?.status !== "ready" || autoProvisionedVersionsRef.current.has(versionId)) return;
    autoProvisionedVersionsRef.current.add(versionId);
    const timer = window.setTimeout(() => void provisionDatabase(true), 350);
    return () => window.clearTimeout(timer);
    // Provisioning is intentionally keyed only by the durable version id.
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [project?.currentVersionId, project?.status]);
  ...
  return (
    <main className={`workbench ${sidebarOpen ? "" : "sidebar-collapsed"}`}>
        ...
        {activeTab === "preview" ? <div className={`preview-stage ${device}`}>
          ...
          {selectedElement && <section className="visual-edit-panel">
            ...
            <div className="visual-fields">
              <label className="visual-wide">文字<input value={visualText} onChange={(event) => setVisualText(event.target.value)} /></label>
              <label>背景<input value={visualBackground} onChange={(event) => setVisualBackground(event.target.value)} placeholder="#ffffff" /></label>
              <label>文字色<input value={visualColor} onChange={(event) => setVisualColor(event.target.value)} placeholder="#111111" /></label>
              <label>圆角<input value={visualRadius} onChange={(event) => setVisualRadius(event.target.value)} placeholder="12px" /></label>
              <label>字号<input value={visualFontSize} onChange={(event) => setVisualFontSize(event.target.value)} placeholder="16px" /></label>
              <label>内边距<input value={visualPadding} onChange={(event) => setVisualPadding(event.target.value)} placeholder="12px 16px" /></label>
              <label>外边距<input value={visualMargin} onChange={(event) => setVisualMargin(event.target.value)} placeholder="0 auto" /></label>
              <label>Gap<input value={visualGap} onChange={(event) => setVisualGap(event.target.value)} placeholder="8px" /></label>
              <label>宽度<input value={visualWidth} onChange={(event) => setVisualWidth(event.target.value)} placeholder="100% / 320px" /></label>
              <label>高度<input value={visualHeight} onChange={(event) => setVisualHeight(event.target.value)} placeholder="auto / 240px" /></label>
              <label>Display<input value={visualDisplay} onChange={(event) => setVisualDisplay(event.target.value)} placeholder="grid / flex" /></label>
              <label>网格列<input value={visualColumns} onChange={(event) => setVisualColumns(event.target.value)} placeholder="1fr 1fr" /></label>
              <label>对齐<input value={visualAlign} onChange={(event) => setVisualAlign(event.target.value)} placeholder="left / center" /></label>
            </div>
            <footer><button onClick={resetVisualPreview}><RotateCcw size={11} />撤销</button><button onClick={applyVisualPreview}>预览修改</button><button className="primary" onClick={persistVisualEdit} disabled={busy}><Sparkles size={12} />写入新版本</button></footer>
          </section>}
      ...
      {showPlatform && <div className="drawer-backdrop"><button className="drawer-dismiss" onClick={() => setShowPlatform(false)} aria-label="关闭全栈资源" /><aside className="version-drawer platform-drawer"><header><div><span>应用全栈资源</span><small>真实 API、Schema、Auth、日志、备份与外部 Runner</small></div><button className="icon-button" onClick={() => setShowPlatform(false)}><X size={18} /></button></header>{platformLoading && !platform ? <div className="platform-loading"><LoaderCircle className="spin" size={20} />正在读取应用控制面…</div> : <div className="platform-content">
        ...
        <section className="platform-section"><header><div><Database size={14} /><strong>每应用物理数据库</strong></div><button onClick={() => void provisionDatabase()}>{platform?.databaseResource?.status === "ready" ? "检查迁移" : "创建 / 重试"}</button></header>{platform?.databaseResource && <div className={`database-resource ${platform.databaseResource.status}`}><span>{platform.databaseResource.status}</span><div><strong>{platform.databaseResource.databaseName}</strong><small>{platform.databaseResource.externalDatabaseId ? `D1 ${platform.databaseResource.externalDatabaseId.slice(0, 12)}…` : "隔离资源尚未由 Provider 创建"} · Schema r{platform.databaseResource.schemaVersion}</small>{platform.databaseResource.lastError && <em>{platform.databaseResource.lastError}</em>}</div></div>}<header><div><Database size={14} /><strong>独立数据 Schema</strong></div><b>{platform?.manifest?.database.isolation ?? "未生成"}</b></header><div className="schema-list">{platform?.manifest?.database.collections.map((collection) => <article key={collection.name}><header><code>{collection.name}</code><span>{collection.access}</span></header><p>{collection.fields.map((field) => `${field.name}:${field.type}${field.required ? "*" : ""}`).join(" · ")}</p></article>) ?? <p>生成首个正式版本后建立 Schema。</p>}</div>{Boolean(platform?.provisioningEvents.length) && <div className="provisioning-audit">{platform?.provisioningEvents.slice(0, 4).map((event) => <span key={event.id}><b>{event.operation}</b><small>{event.status} · {new Date(event.startedAt).toLocaleString("zh-CN")}</small></span>)}</div>}</section>
        ...
        <section className="platform-section"><header><div><UserRound size={14} /><strong>组织、权限、审批与用量</strong></div><b>{platform?.organization?.plan ?? "demo"}</b></header><div className="organization-meter"><span><b>{(platform?.organization?.usage.tokens ?? 0).toLocaleString("zh-CN")}</b><small>/ {(platform?.organization?.monthlyTokenLimit ?? 0).toLocaleString("zh-CN")} Tokens</small></span><span><b>{platform?.organization?.members.length ?? 0}</b><small>成员</small></span><span><b>{platform?.approvals.filter((item) => item.status === "pending").length ?? 0}</b><small>待审批</small></span></div><div className="platform-inline-form"><input type="email" value={inviteEmail} onChange={(event) => setInviteEmail(event.target.value)} placeholder="协作者邮箱" /><button onClick={() => void inviteMember()}>邀请为编辑</button></div><div className="approval-actions"><button onClick={() => void toggleApprovalPolicy()}>{platform?.organization?.approvalRequired ? "关闭发布审批" : "开启发布审批"}</button>{platform?.organization?.approvalRequired && <button onClick={() => void requestApproval()} disabled={!hasSavedVersion}>提交当前版本审批</button>}</div><div className="member-chips">{platform?.organization?.members.map((member) => <span key={member.id}><b>{member.email ?? (member.role === "owner" ? "当前所有者" : member.subjectId.slice(0, 12))}</b><small>{member.role} · {member.status}</small></span>)}</div>{platform?.notifications[0] && <div className={`delivery-status ${platform.notifications[0].status}`}><b>最近邮件：{platform.notifications[0].status}</b><small>{platform.notifications[0].recipient}{platform.notifications[0].error ? ` · ${platform.notifications[0].error}` : ""}</small></div>}</section>
        <section className="platform-section"><header><div><HardDrive size={14} /><strong>Stripe 套餐、发票与超额用量</strong></div><b>{platform?.billing?.status ?? "configuration-required"}</b></header><div className="billing-actions"><button onClick={() => void openCheckout("team")}>开通 Team</button><button onClick={() => void openCheckout("enterprise")}>开通 Enterprise</button><button onClick={() => void openBillingPortal()} disabled={!platform?.billing?.customerId}>账单门户</button></div>{Boolean(platform?.invoices.length) && <div className="member-chips">{platform?.invoices.slice(0, 3).map((invoice) => <span key={invoice.id}><b>{invoice.status} · {(invoice.amountDue / 100).toLocaleString("zh-CN", { style: "currency", currency: invoice.currency.toUpperCase() })}</b><small>{new Date(invoice.createdAt).toLocaleDateString("zh-CN")}{invoice.hostedInvoiceUrl && <> · <a href={invoice.hostedInvoiceUrl} target="_blank" rel="noreferrer">查看发票</a></>}</small></span>)}</div>}<small className="platform-help">Checkout、签名 Webhook、发票镜像、Billing Portal 与 Meter Event 均走服务端链路；未配置 Stripe Secret 时不会创建假订单。</small></section>
        <section className="platform-section"><header><div><Code2 size={14} /><strong>GitHub App 与 Agent 分支</strong></div><b>{platform?.git?.status ?? "未连接"}</b></header><div className="github-app-row"><a href="/api/github/app/install">安装 GitHub App</a><span>{platform?.githubInstallations.length ? `${platform.githubInstallations[0].accountLogin} · Installation ${platform.githubInstallations[0].installationId}` : "尚未验证 Installation"}</span></div><div className="platform-inline-form"><input value={gitRepository} onChange={(event) => setGitRepository(event.target.value)} placeholder="owner/repository" /><button onClick={() => void configureGit()}>连接</button></div>{platform?.git && <div className="git-sync-card"><div><strong>{platform.git.repositoryOwner}/{platform.git.repositoryName}</strong><small>默认分支 {platform.git.defaultBranch} · {platform.git.installationId ? `GitHub App #${platform.git.installationId}` : "PAT 兼容模式"}</small></div><button onClick={() => void syncGit()} disabled={platform.git.status !== "connected"}>创建并合并</button></div>}{platform?.git?.lastSync && <div className="member-chips">{platform.git.lastSync.branches.map((branch) => <span key={branch.branch}><b>{branch.agent}</b><small>{branch.branch}</small></span>)}</div>}</section>
        <section className="platform-section"><header><div><HardDrive size={14} /><strong>数据库备份与回滚</strong></div><button onClick={() => void createBackup()}>立即备份</button></header><div className="backup-list">{platform?.backups.length ? platform.backups.map((backup) => <article key={backup.id}><div><strong>{backup.label}</strong><small>{backup.recordCount} 条记录 · {new Date(backup.createdAt).toLocaleString("zh-CN")} · {backup.provider === "cloudflare-d1-time-travel" ? "Time Travel" : "逻辑快照"}</small><small>长期归档：{backup.archiveStatus ?? "not-configured"}{backup.archiveBytes ? ` · ${(backup.archiveBytes / 1024).toFixed(1)} KB` : ""}{backup.archiveError ? ` · ${backup.archiveError}` : ""}</small></div><button onClick={() => void restoreDataBackup(backup.id)}>恢复</button></article>) : <p>还没有数据备份。</p>}</div></section>
        ...
        <section className="platform-section"><header><div><Activity size={14} /><strong>SLO / APM / 告警</strong></div><b>{platform?.operations.slo.healthy ? "healthy" : "breached"}</b></header><div className="operations-grid"><span><b>{((platform?.operations.errorRate ?? 0) * 100).toFixed(2)}%</b><small>60 分钟错误率</small></span><span><b>{platform?.operations.p95DurationMs ?? "—"}</b><small>P95 ms</small></span><span><b>{platform?.operations.alerting ?? "configuration-required"}</b><small>Sentry / Webhook</small></span></div></section>
      </div>}</aside></div>}
